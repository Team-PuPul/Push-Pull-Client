...
PR 본문은 **짧게** 쓴다. 리뷰어가 훑어보고 바로 이해할 수 있는 길이가 목표다.
 
- **전체 20줄 이내**를 기본으로 한다.
- **개요**: 1~2문장. 무엇을 왜 바꿨는지만.
- **내용**: 변경 영역별로 **한 줄 요약**. 소제목(`##`)을 남발하지 말고 불릿으로 정리한다.
- **기타 사항**: 리뷰어가 반드시 알아야 할 것만. 없으면 `없음`.
 
**하지 말 것:**
- 코드 동작을 단계별로 서술하거나 호출 흐름(`A() → B() → C()`)을 길게 설명하지 않는다
- 원인 분석·디버깅 과정을 옮겨 적지 않는다 (그건 커밋 메시지와 코드 주석의 몫)
- 변경한 파일을 하나하나 나열하지 않는다 (diff에 이미 있다)
...
- 위 **분량 규칙**을 지킨다 (전체 20줄 이내, 각 항목은 한 줄 요약)
...
# 내용
<!-- 변경 영역별 한 줄 요약. 불릿으로 정리 -->
- <변경 영역>: <무엇을 어떻게 바꿨는지 한 줄>
- <변경 영역>: <무엇을 어떻게 바꿨는지 한 줄>
 
# 기타 사항
<!-- 리뷰어가 반드시 알아야 할 것만. 없으면 "없음" -->
```
...
일시정지 UI를 닫아도 버튼이 네비게이션 대상으로 남아, 씬 재시작 후 이동할 때 UI 선택 사운드가 재생되던 문제를 수정했습니다.
 
# 내용
 
- `ButtonPanel`: 캔버스가 화면에 없으면 첫 버튼을 자동 선택하지 않음
- `ButtonCanvas`: 숨긴 캔버스는 `CanvasGroup.interactable`을 끔
- `UIButton`: 상호작용 불가 버튼은 호버 사운드 무음
- `SettingCanvas` 프리팹: 닫힌 상태(Canvas 비활성)로 저장
 
# 기타 사항
 
설정 화면 진입·이탈이 정상 동작하는지 함께 확인 부탁드립니다.
```
